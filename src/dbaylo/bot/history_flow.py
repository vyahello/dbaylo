...
async def on_chart_pdf(callback: CallbackQuery) -> None:
    ...
    # EVERY indicator gets a description — but a note is data-independent, so it's generated by
    # claude only ONCE EVER and PERSISTED (notecache): read the cache, generate only the misses
    # (bounded so a slow claude can't hang it), persist them, then assemble. After the cache is warm
    # (browsing single charts warms it too) the PDF needs no claude call at all — render is ~1s.
    ...
    chart_notes = [_note_for(d.title, d.specimen) for d in data]
    qual_notes = [_note_for(q.title, q.specimen) for q in quals]
