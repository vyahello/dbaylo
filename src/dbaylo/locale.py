...
PRESCRIPTION_LINE_DURATION = (
    "курс {duration}"  # the doctor's term; after it the bot stops reminding
)
...
NAV_PRICE_FREEFORM_QUERY = (
    "Користувач написав довільний запит про ціну ліків. Визнач, які САМЕ названі ліки (і "
    "дозування / місто, якщо назвав) він має на увазі, і знайди ціни за правилами вище. Якщо вище "
    "вже є попередня розмова — памʼятай, про які ліки й місто йшлося, і не перепитуй назву на "
    "коротке уточнення («а дешевше?», «а в Києві?»). НЕ вибирай ліки за симптомом — лише названі "
    "явно. Якщо ліків так і не названо — коротко попроси назвати конкретні ліки.\n"
    "Місто: {city}.\nНове повідомлення користувача: {request}"
)
# Prepended when the price conversation continues — the agent must remember the drug + city.
NAV_PRICE_FREEFORM_HISTORY = (
    "Це ПРОДОВЖЕННЯ розмови про ціни. Попередні репліки (памʼятай контекст — про які ліки, "
    "дозування й місто вже йшлося):\n{history}\n\n"
)
...
# the numbers to course names.
BTN_PRICE_MED = "{mark} {name}"
BTN_PRICE_MED_DOSE = "{mark} {name} · {dose}"  # a saved med WITH its strength
...
NAV_PRICE_LEGEND_HEADER = "Твої рецепти:"
NAV_PRICE_COURSE_LEGEND = "{mark} {course}"
...
MENU_CARE_INTRO = (
    "💊 Твої ліки та нагадування про них.\n\n"
    "📷 Маєш рецепт (лист призначень)? Просто надішли фото або PDF — я сам зчитаю ліки, "
    "дозування й час прийому, а ти підтвердиш."
)
...
# Sent with parse_mode=HTML (the one menu intro that is) — a styled title + italic subtitle, then
# the two options as bold-led lines that map 1:1 to the two buttons below.
MENU_PRICES_INTRO = (
    "💰 <b>Ціни та державні гарантії</b>\n"
    "<i>Скільки коштує — і що можна отримати безкоштовно від держави.</i>\n"
    "\n"
    "💊 <b>Ціна ліків</b> — реальні ціни в аптеках із прямими посиланнями.\n"
    "🆓 <b>Безкоштовно (ПМГ)</b> — які послуги й ліки покриває держава: що для цього "
    "треба й де це поруч."
)
...
MED_CARD_UNTIL = "⏳ Курс до {until} (далі нагадування зупиню)"  # the term, if the doctor set one
...
COURSE_CARD_ITEM_UNTIL = "💊 {name} · {times} · до {until}"  # a med with a term (course end date)
COURSE_CARD_HINT = "Нагадування приходять окремо — кожне у свій час. 💊"
# Sent when a med's prescribed term passes — the bot stops reminding and drops it from the list.
MED_COURSE_FINISHED = (
